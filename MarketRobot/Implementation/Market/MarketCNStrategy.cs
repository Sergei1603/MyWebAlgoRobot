...
{
    internal class MarketCNStrategy : MarketStrategyAbstraction, IMarketCNStrategy
    {

        public MarketCNStrategy(ITinkoffProvider tinkoffProvider, ILogger logger)
        {
            _tinkoffProvider = tinkoffProvider;
            _logger = logger;

            _basiсAktive = "CNY/RUB";
        ...
        protected override TimePeriod SetDate()
        {
            var _from = TimeZoneInfo.ConvertTimeToUtc(new DateTime(DateTime.Today.Year, 
                DateTime.Today.Month, DateTime.Today.Day, 10, 5, 0), _moscowZone);
            var _to = TimeZoneInfo.ConvertTimeToUtc(new DateTime(DateTime.Today.Year, 
                DateTime.Today.Month, DateTime.Today.Day, 10, 15, 0), _moscowZone);
            ...
            try
            {
                ...
                if (historyCandels is not null && nowCandels is not null)
                {
                    ...
                    if (_value > 0)
                    {
                        ...
                        else if (deltaPrice <= (decimal)-0.2)
                        {
